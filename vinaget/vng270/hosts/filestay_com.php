...
class dl_filestay_com extends Download {
    ...
    public function Leech($url) {
		...
		if($pass) {
			...
			$giay = $this->lib->cut_str($this->lib->cut_str($data, '<a style="margin-top:15px;"', 'active">Download'), 'href="', '" class="gbutton large bold positive');
			return trim($giay);
		}
		...
		elseif(stristr($data, "Create Download Link")){
			$post = $this->parseForm($this->lib->cut_str($data, '<Form name="F1"', '</Form>'));
			$data = $this->lib->curl($url, $this->lib->cookie, $post);
			$link = $this->lib->cut_str($this->lib->cut_str($data, '<a style="margin-top:15px;"', 'active">Download'), 'href="', '" class="gbutton large bold positive');
			return trim($link);
		}
